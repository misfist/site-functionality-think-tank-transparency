...
class Actions extends Base {
	 ...
	 * @return void
	 */
	public function set_donor_parent( int $post_id ): void {
	 ...
	 */
	public function set_transaction_donor_data( int $post_id ): void {
	...
	public function before_import( int $import_id ): void {
		$batch_size = 100;
		$this->delete_posts( $import_id, $batch_size );
	}
	...
	public function set_transactions_data() : void {
		...
		if ( ! empty( $transactions ) && ! \is_wp_error( $transactions ) ) {
			foreach ( $transactions as $transaction_id ) {
				$this->set_transaction_data( $transaction_id );
			}
		}
	}
	 ...
	 * @return void
	 */
	public function process_think_tanks(): void {
	...
	public function delete_post_term( int $post_id ): void {
		...
		$taxonomy  = $post_type;
		$term_id   = $this->get_term_id_from_post_id( $post_id, $taxonomy );
		if ( false !== $term_id ) {
	 ...
	 * }
	 */
	public function get_think_tank_sums( string $think_tank = '', string $donor_type = '' ): array {
		$post_ids = $this->get_think_tank_post_ids( $think_tank, $donor_type );

		return array(
			'amount_calc' => $this->get_total( $post_ids ),
			'undisclosed' => $this->is_undisclosed( $post_ids ),
		);
	}
	 ...
	 */
	public function get_think_tank_post_ids( string $think_tank = '', string $donor_type = '' ): array {
		$args = array(
			'post_type'      => 'transaction',
			'posts_per_page' => -1,
			'tax_query'      => array(),
	 ...
	 * }
	 */
	public function get_donor_sums( string $donor = '' ): array {
		$post_ids = $this->get_donor_post_ids( $donor );
	 ...
	 * @return array Array of post IDs.
	 */
	public function get_donor_post_ids( string $donor = '' ): array {
		$args = array(
			'post_type'      => 'transaction',
			'posts_per_page' => -1,
			'tax_query'      => array(),
	 ...
	 */
	public function get_total( array $post_ids ): int {
		if ( empty( $post_ids ) ) {
			return 0;
		}
	 ...
	 */
	public function is_undisclosed( array $post_ids ): bool {
		if ( empty( $post_ids ) ) {
			return false;
		}
